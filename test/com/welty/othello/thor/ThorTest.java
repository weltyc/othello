...
public class ThorTest extends TestCase {
    ...
    public static void testThorFindMatchingPositions() {
        ...
        TestCase.assertEquals(0, matchingPositions.index.size());
        ...
        TestCase.assertEquals(1, matchingPositions.index.size());
        TestCase.assertEquals(1, matchingPositions.iReflections.size());
        TestCase.assertEquals(0, matchingPositions.index.get(0));
    ...
    public static void testThorFindNextMoves() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            assertEquals("" + i, f5Reflection.BitBoard().MinimalReflection(), f5.BitBoard().MinimalReflection());
        }
    }
    ...
    public static void testThorSummarize() {
        ...
        matchingPositions = Thor.ThorFindMatchingPositions(games, osGame.pos.board);
        TestCase.assertEquals(3, matchingPositions.index.size());
        summary = Thor.ThorSummarize(games, osGame.pos.board, matchingPositions.index, matchingPositions.iReflections);
        assertEquals(0, summary.size());
        ...
        summary = Thor.ThorSummarize(games, osGame.pos.board, matchingPositions.index, matchingPositions.iReflections);
        assertEquals(1, summary.size());
        TestCase.assertNotNull(summary.get(-1));
    }
    ...
    private static void checkMove(TIntObjectHashMap<ThorSummaryData> summary, byte move, int nPlayed, int nBlackWins, int nWhiteWins) {
        ...
        TestCase.assertNotNull(summary.get(move));
        data = summary.get(move);
        TestCase.assertEquals(nPlayed, data.nPlayed);
        TestCase.assertEquals(nBlackWins, data.nBlackWins);
        TestCase.assertEquals(nWhiteWins, data.nWhiteWins);
    }
